...
// const { MyFiLedgerContract } = require('..');
const winston = require('winston');
...
describe('MyFiLedgerContract', () => {

    let contract;
    let ctx;

    beforeEach(() => {
        contract = new MyFiLedgerContract();
        ctx = new TestContext();
        ctx.stub.getState.withArgs('1001').resolves(Buffer.from('{"value":"my document 1001 value"}'));
        ctx.stub.getState.withArgs('1002').resolves(Buffer.from('{"value":"my document 1002 value"}'));
    });

    describe('#myAssetExists', () => {

        it('should return true for a my document', async () => {
            await contract.myDocExists(ctx, '1001').should.eventually.be.true;
        });

        it('should return false for a my document that does not exist', async () => {
            await contract.myDocExists(ctx, '1003').should.eventually.be.false;
        });
    ...
    describe('#createMyAsset', () => {
        ...
        it('should create a my document', async () => {
            await contract.createDoc(ctx, '1003', 'my document 1003 value');
            ctx.stub.putState.should.have.been.calledOnceWithExactly('1003', Buffer.from('{"value":"my document 1003 value"}'));
        });

        it('should throw an error for a my document that already exists', async () => {
            await contract.createDoc(ctx, '1001', 'myvalue').should.be.rejectedWith(/The my document 1001 already exists/);
        });
    ...
    describe('#readMyAsset', () => {
        ...
        it('should return a my document', async () => {
            await contract.readDoc(ctx, '1001').should.eventually.deep.equal({ value: 'my document 1001 value' });
        });

        it('should throw an error for a my document that does not exist', async () => {
            await contract.readDoc(ctx, '1003').should.be.rejectedWith(/The my document 1003 does not exist/);
        });
    ...
    describe('#updateMyAsset', () => {
        ...
        it('should update a my document', async () => {
            await contract.updateDoc(ctx, '1001', 'my document 1001 new value');
            ctx.stub.putState.should.have.been.calledOnceWithExactly('1001', Buffer.from('{"value":"my document 1001 new value"}'));
        });

        it('should throw an error for a my document that does not exist', async () => {
            await contract.updateDoc(ctx, '1003', 'my document 1003 new value').should.be.rejectedWith(/The my document 1003 does not exist/);
        });
    ...
    describe('#deleteMyAsset', () => {
        ...
        it('should delete a my document', async () => {
            await contract.deleteDoc(ctx, '1001');
            ctx.stub.deleteState.should.have.been.calledOnceWithExactly('1001');
        });

        it('should throw an error for a my document that does not exist', async () => {
            await contract.deleteDoc(ctx, '1003').should.be.rejectedWith(/The my document 1003 does not exist/);
        });
